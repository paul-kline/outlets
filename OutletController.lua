...
srv:listen(80,function(conn)
    ...
    conn:on("receive", function(client,request)
        ...
        if(_GET ~= nil)then
           ...
           if(_GET.configMode~=nil)then
             if(_GET.configMode == "configMode")then
        ...
        buf = buf.."<!DOCTYPE html> <html><head><title>Outlet Controller</title> </head><body> <h1> Outlet Controller</h1>";
        ...
        buf = buf..[[<br><br><br><br><a href="/?configMode=configMode"><button>Change Configuration</button></a></body></html>]]
        local _on,_off = "",""
        if(pin0State)then
        -- this relay is opposite type. 
        print("turning pin: "..pin0.." to high")
             gpio.write(pin0, gpio.HIGH);
        else 
             gpio.write(pin0, gpio.LOW);
        end
